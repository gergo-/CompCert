...
Definition fe_ofs_arg := 16.

(** Computation of the frame environment from the bounds of the current
  function. *)

Definition make_env (b: bounds) :=
  let ol := align (fe_ofs_arg + 4 * b.(bound_outgoing)) 8 in    (* locals *)
...
Proof.
  ...
  set (ostkdata := align (olink + 4) 8).
  generalize b.(bound_local_pos) b.(bound_outgoing_pos) b.(bound_stack_data_pos); intros.
  assert (0 <= fe_ofs_arg + 4 * b.(bound_outgoing)) by (unfold fe_ofs_arg; omega).
  assert (fe_ofs_arg + 4 * b.(bound_outgoing) <= ol) by (apply align_le; omega).
  assert (ol + 4 * b.(bound_local) <= ocs) by (unfold ocs; omega).
  assert (ocs <= size_callee_save_area b ocs) by apply size_callee_save_area_incr.
  assert (size_callee_save_area b ocs <= ora) by (apply align_le; omega).
