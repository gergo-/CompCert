...
Remark transl_memory_access_label:
  forall (mk_instr_imm: gpreg -> int -> instruction)
  ...
  (match mk_instr_gen with
    | None => True
    | Some f => forall r n, nolabel (f r n)
   end) ->
  tail_nolabel k c.
Proof.
...
Lemma exec_straight_steps:
  ...
  (forall k c (TR: transl_instr f i ep k = OK c),
   exists rs2,
    ...
    /\ agree ms2 sp rs2
    /\ (it1_is_parent ep i = true -> rs2#SP = parent_sp s)) ->
  exists st',
...
(* FIXME: Should be in Asmgenproof1 *)
...
Proof.
  intros. destruct a; try discriminate H.
  simpl. rewrite Ptrofs.of_int_to_int by auto.
  assumption.
Qed.
...
- (* Mtailcall *)
  ...
  simpl in H7. unfold bind in H7. unfold it1_is_parent in H7.
  destruct (transl_code f c false) in H7; inversion H7.
(*
...
- (* Mreturn *)
  ...
  exploit Mem.free_parallel_extends; eauto. intros [m2' [E F]].
  monadInv H6.
  set (rs2 := nextinstr (rs0#GPR10 <- (parent_ra s))).
  set (rs3 := nextinstr (rs2#RA <- (parent_ra s))).
           ...
           (Pret :: x) rs4 m2').
  {
    ...
    - simpl. unfold exec_load. erewrite loadv_offset_ptr by eexact C. auto.
    - simpl. auto.
    - simpl. change (rs3 SP) with (rs0 SP).
      rewrite A. rewrite <- (sp_val _ _ _ AG). rewrite E. auto.
    - auto.
    - auto.
    - auto.
  }
  ...
  repeat eapply code_tail_next_int; eauto.
  reflexivity. traceEq.
  (* match states *)
  econstructor; eauto.
  (*
  split. Simpl. eapply parent_sp_def; eauto.
   *)
  (*simpl. Simpl. eapply parent_sp_def.*)
  assert (AG3: agree rs (Vptr stk soff) rs3).
  {
    unfold rs3, rs2.
    repeat apply agree_nextinstr, agree_set_other; auto.
  }
...
- (* internal function *)
  ...
  set (rs2 := nextinstr (rs0#SP <- sp (*#GPR10R11 <- Vundef*))).
  set (rs3 := nextinstr (rs2#GPR10 <- (rs0#RA))).
              ...
              x0 rs4 m3').
  {
    ...
    unfold exec_instr. rewrite C. fold sp.
      rewrite <- (sp_val _ _ _ AG). unfold Tptr, chunk_of_type, Archi.ptr64 in F. rewrite F. auto. auto.
    apply exec_straight_step with rs3 m2'. simpl. auto. auto.
    apply exec_straight_one.
      simpl. unfold exec_store.
      rewrite Ptrofs.add_zero_l. rewrite Ptrofs.of_int_to_int. simpl.
      simpl in P. rewrite Ptrofs.add_zero_l in P.
      change (rs3 GPR10) with (rs0 RA).
      rewrite ATLR. rewrite P. auto. 
    auto.
    auto.
  }
  ...
  unfold rs4, rs3, rs2.
  apply agree_nextinstr. apply agree_nextinstr. apply agree_set_other; auto.
