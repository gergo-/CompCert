...
Lemma contains_get_stack:
  ...
  m |= contains (chunk_of_type (typ_of_quantity q)) sp ofs spec ->
...
Lemma hasvalue_get_stack:
  forall q m sp ofs v,
  m |= hasvalue (chunk_of_type (typ_of_quantity q)) sp ofs v ->
...
Lemma contains_set_stack:
  ...
  m |= contains (chunk_of_type (typ_of_quantity q)) sp ofs spec1 ** P ->
  spec (Val.load_result (chunk_of_type (typ_of_quantity q)) v) ->
  exists m',
      store_stack m (Vptr sp Ptrofs.zero) q (Ptrofs.repr ofs) v = Some m'
  /\ m' |= contains (chunk_of_type (typ_of_quantity q)) sp ofs spec ** P.
Proof.
...
Program Definition contains_locations (j: meminj) (sp: block) (pos bound: Z) (sl: slot) (ls: locset) : massert := {|
  m_pred := fun m =>
    ...
    exists v, Mem.load (chunk_of_type (typ_of_quantity q)) m sp (pos + 4 * ofs) = Some v
...
Remark valid_access_location:
  ...
  Mem.valid_access m (chunk_of_type (typ_of_quantity q)) sp (pos + 4 * ofs) p.
Proof.
  intros; split.
...
- rewrite align_type_chunk. apply Z.divide_add_r.
  ...
  exists (8 / (4 * typealign (typ_of_quantity q))); destruct q; reflexivity.
...
- simpl; intuition auto. red; intros; eauto with mem.
  destruct (Mem.valid_access_load m (chunk_of_type (typ_of_quantity q)) sp (pos + 4 * ofs)) as [v LOAD].
  eapply valid_access_location; eauto.
  red; intros; eauto with mem.
...
Proof.
  ...
  unfold return_regs_spec. destruct l; auto. rewrite H; auto.
Qed.
...
- set (ty := typ_of_quantity (quantity_of_typ (mreg_type r))) in *.
  ...
  rewrite (typ_of_quantity_of_typ (mreg_type r) (mreg_type_cases r)).
  ...
  rewrite (typ_of_quantity_of_typ (mreg_type r) (mreg_type_cases r)) in B. exact B.
...
Local Opaque b fe.
  ...
  eexact SEP. apply Val.load_result_same; auto using has_typ_of_quantity.
...
(* Conclusions *)
  ...
  split. rewrite LS1. apply agree_locs_undef_locs; [|reflexivity].
    ...
    rewrite call_regs_correct. apply AGCS. auto.
...
Proof.
  ...
  exploit (hasvalue_get_stack (quantity_of_typ Tptr)). eapply sep_pick3; eexact SEP. intros LOAD_LINK.
  exploit (hasvalue_get_stack (quantity_of_typ Tptr)). eapply sep_pick4; eexact SEP. intros LOAD_RETADDR.
  clear SEP.
  (* Conclusions *)
...
Proof.
  ...
  destruct (mreg_type r); simpl; rewrite Maps.ZMap.gi; auto.
  red; simpl; auto.
