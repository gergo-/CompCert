...
Proof.
  intros; red; intros l'.
  destruct (Loc.eq l l').
  rewrite e, Locmap.gss. destruct (chunk_of_type (Loc.type l')); simpl; auto.
  destruct (Loc.diff_dec l l').
  rewrite Locmap.gso; auto.
  rewrite Locmap.gu_overlap; auto using Loc.diff_sym.
Qed.
...
Proof.
  ...
  destruct (Conventions1.is_callee_save r). exact (H (R r)). exact (H0 (R r)).
  destruct caller1, caller2. exact (H (S sl pos q)).
Qed. 
